...
private taskUrl = 'api/tasks';  // URL to web API
 ...
 /*getTasks():Promise<Task[]> {
  return Promise.resolve(TASKS);
	...
	getTaskApi (sprintId:any): Observable<Task[]> {
	...
	addTask (name: string ,_id:string, pri:number,desc:string,type:string,asignId:string,start:string,end:string): Observable<Task> {
		...
		return this.http.post(this.taskUrl+'/addTasks', { name ,_id ,pri,desc,type,asignId,start,end}, options)
	...
	deleteTask(task_id:any):Observable<Task> {		
